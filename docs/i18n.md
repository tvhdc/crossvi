...
This guide explains the multi-language support system in CrossVi.

Production firmware intentionally compiles only the languages listed in
[`lib/I18n/build-languages.txt`](../lib/I18n/build-languages.txt). See
[`build-languages.md`](./build-languages.md) for the exact list, compatibility rules, and how to enable or disable a UI
language without deleting its translation source.
...
```
lib/I18n/
├── build-languages.txt          # Production UI-language whitelist
...
**Key principle:** All translations are managed in the YAML files under `lib/I18n/translations/`. The production subset is
configured only in `lib/I18n/build-languages.txt`; the Python script generates the necessary C++ code from that subset.
...
- `_order` — Legacy catalog metadata. Production enum order is controlled by `lib/I18n/build-languages.txt`.
...
#### 3. Enable it in production

Add its `_language_code` to `lib/I18n/build-languages.txt`; see [build-languages.md](./build-languages.md).
...
On load, current firmware retires that ambiguous numeric file after publishing a stable language code. Languages not
included in the production build fall back safely to English.
