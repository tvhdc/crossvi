...
bool SdCardFont::computeStyleFileOffsets(PerStyle& s, const uint32_t baseOffset, const uint32_t styleEnd) {
  ...
  if (!checkedAddMul(next, s.header.intervalCount, sizeof(EpdUnicodeInterval), next) || next > UINT32_MAX) return false;
  ...
  if (!checkedAddMul(next, s.header.ligaturePairCount, sizeof(EpdLigaturePair), next) || next > styleEnd) return false;
...
bool SdCardFont::load(const char* path) {
  ...
  for (uint8_t i = 0; i < styleCount; i++) {
    ...
    const bool invalidKernShape = (s.header.kernLeftEntryCount == 0) != (s.header.kernLeftClassCount == 0) ||
  ...
  for (uint8_t i = 0; i < MAX_STYLES; i++) {
    ...
    for (uint32_t j = 0; j < s.header.intervalCount; ++j) {
      ...
      expectedOffset += span;
      prevLast = iv.last;
      if ((j & 0x7FU) == 0) serviceLongValidation();
    }
    ...
    for (uint32_t glyphIndex = 0; glyphIndex < s.header.glyphCount; ++glyphIndex) {
      ...
      if ((glyphIndex & 0x7FU) == 0) serviceLongValidation();
    }
    ...
    auto validateKernTable = [&](const uint32_t offset, const uint16_t count, const uint8_t classCount,
      ...
      for (uint16_t entryIndex = 0; entryIndex < count; ++entryIndex) {
        ...
        if ((entryIndex & 0x7FU) == 0) serviceLongValidation();
      }
      return true;
    };
