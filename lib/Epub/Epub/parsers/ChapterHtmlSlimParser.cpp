...
constexpr size_t TEXT_BLOCK_SOFT_FLUSH_WORDS = 512;
...
void ChapterHtmlSlimParser::failOutOfMemory(const char* stage) {
  if (lastFailure_ == ChapterParseFailure::OutOfMemory) return;
...
bool ChapterHtmlSlimParser::ensureMemory(const char* stage) {
  if (lastFailure_ == ChapterParseFailure::OutOfMemory) return false;
  auto memory = MemoryBudget::snapshot();
  if (MemoryBudget::hasHeadroom(memory, MemoryBudget::TEXT_LAYOUT)) return true;

  if (!releasedFontCachesForMemory_) {
    releasedFontCachesForMemory_ = true;
    if (auto* cache = renderer.getFontCacheManager()) cache->clearAllCaches();
    memory = MemoryBudget::snapshot();
    if (MemoryBudget::hasHeadroom(memory, MemoryBudget::TEXT_LAYOUT)) return true;
  }

  failOutOfMemory(stage);
  return false;
}

bool ChapterHtmlSlimParser::createPage(const char* stage) {
  if (!ensureMemory(stage)) return false;
...
void ChapterHtmlSlimParser::applySmallCapsToEntry(StyleStackEntry& entry, const CssStyle& css) {
  if (!css.hasFontVariantCaps()) return;
...
void ChapterHtmlSlimParser::updateEffectiveInlineStyle() {
  ...
  for (const auto& entry : inlineStyleStack) {
    ...
    if (entry.hasSmallCaps) effectiveSmallCaps = entry.smallCaps;
  }
...
void ChapterHtmlSlimParser::flushPendingAnchor() {
  ...
  if (std::find(tocAnchors.begin(), tocAnchors.end(), pendingAnchorId) != tocAnchors.end()) {
    if (currentPage && !currentPage->elements.empty()) {
      ...
      completedPageCount++;
      if (!createPage("toc_page_break")) return;
    }
  }
...
void ChapterHtmlSlimParser::flushPartWordBuffer() {
  if (!currentTextBlock || !ensureMemory("token")) {
  ...
  if (canonicalTextOffsetValid) {
    if (sourceBytes > UINT32_MAX - canonicalTextOffset) {
      ...
      canonicalTextOffset += static_cast<uint32_t>(sourceBytes);
    }
  }
  partWordBufferIndex = 0;
  nextWordContinues = false;
  listItemBulletOnly = false;
  softFlushTextBlockIfNeeded();
}
...
void ChapterHtmlSlimParser::softFlushTextBlockIfNeeded() {
  ...
  LOG_DBG("EHP", "Text block soft flush (%u words)", static_cast<unsigned>(currentTextBlock->size()));
...
void ChapterHtmlSlimParser::startNewTextBlock(const BlockStyle& blockStyle) {
  ...
  // block is flushed so the chapter starts on a fresh page.
  flushPendingAnchor();
  if (!ensureMemory("text_block")) return;
...
void ChapterHtmlSlimParser::emitHorizontalRule(const BlockStyle& blockStyle) {
  ...
  if (!currentPage) {
    if (!createPage("horizontal_rule_page")) return;
  }
  ...
  if (!currentPage->elements.empty() && currentPageNextY + totalHeight > viewportHeight) {
    ...
    completePageFn(std::move(currentPage), xpathParagraphIndex, xpathListItemIndex, currentPageVisibleOffset);
    completedPageCount++;
    if (!createPage("horizontal_rule_break")) return;
  }
...
void XMLCALL ChapterHtmlSlimParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
      ...
      if (!src.empty() && self->imageRendering != 1) {
...

        {
          ...
          if (ImageDecoderFactory::isFormatSupported(resolvedPath)) {
            ...
            // deferred until the image is actually rendered.
            ImageDimensions dims = {0, 0};
            ImageDimsProbe headerProbe;
            const bool probeRead = self->epub->readItemContentsToStream(resolvedPath, headerProbe, 1024, true);
            bool gotDimensions = probeRead && headerProbe.getDimensions(dims);

            // Malformed or unusual headers use the old, fully validated path.
            // This keeps unsupported/corrupt images fail-closed without making
            // the common JPEG/PNG path pay the full inflate cost during indexing.
            if (!gotDimensions) {
              if (!self->epub->extractItemToFileAtomically(resolvedPath, cachedImagePath)) {
            ...
            if (gotDimensions) {
              ...
              if (self->currentPage && !self->currentPage->elements.empty() &&
                ...
                if (!self->createPage("image_page")) return;
              }

              // A full-height image must not inherit positive top spacing on
              // its fresh page: ImageBlock correctly rejects off-screen
              // bounds, which otherwise leaves an empty page behind.
              const int maxImageMarginTop = std::max(0, self->viewportHeight - self->currentPageNextY - displayHeight);
              if (imageMarginTop > maxImageMarginTop) imageMarginTop = static_cast<int16_t>(maxImageMarginTop);

              // Apply top margin from container block
              self->currentPageNextY += imageMarginTop;

              // Create ImageBlock and add to page
              // Retain the normalized source href even when the dimension
  ...
  if (matches(name, HEADER_TAGS, std::size(HEADER_TAGS))) {
    ...
    if (strcmp(name, "br") == 0) {
      ...
      // margins at a <br>. A standalone or consecutive <br> keeps the container
      // margins so its empty block still acts as a scene-break separator.
      // Always start from the open parent style; the current block may belong to an
      // element that has already closed and must not leak alignment or margins.
      ...
      self->currentCssStyle = cssStyle;
      const auto accumulated =
          self->blockStyleStack.back().getCombinedBlockStyle(effectiveBlockStyle, BlockStyle::CombineAxis::Horizontal);
      self->blockStyleStack.push_back(accumulated);
      self->startNewTextBlock(accumulated.withoutBottom());
      self->updateEffectiveInlineStyle();

      if (strcmp(name, "li") == 0) {
        self->currentTextBlock->addWord("\xe2\x80\xa2", EpdFontFamily::REGULAR, false, false, UINT32_MAX,
    ...
    if (cssStyle.hasFontWeight() || cssStyle.hasFontStyle() || cssStyle.hasTextDecoration() ||
        cssStyle.hasDirection() || cssStyle.hasVerticalAlign() || cssStyle.hasFontVariantCaps()) {
      // Flush buffer before style change so preceding text gets current style
      if (self->partWordBufferIndex > 0) {
        self->flushPartWordBuffer();
        self->nextWordContinues = true;
...
void XMLCALL ChapterHtmlSlimParser::characterData(void* userData, const XML_Char* s, const int len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (self->partWordBufferIndex == 0) self->partWordVisibleOffset = codepointOffset;
...
void XMLCALL ChapterHtmlSlimParser::endElement(void* userData, const XML_Char* name) {
  ...
  if (headerOrBlockTag) {
    ...
    if (strcmp(name, "br") != 0 && self->blockStyleStack.size() > 1) {
      ...
      // Bare text following a closed block belongs to the parent, not to the
      // alignment or margins of the element that just ended.
      self->startNewTextBlock(self->blockStyleStack.back());
    }
...
bool ChapterHtmlSlimParser::beginParse() {
  ...
  if (!ensureMemory("parse_begin")) return false;
...
ChapterHtmlSlimParser::ParseStatus ChapterHtmlSlimParser::parseStep() {
  if (!ensureMemory("parse_step")) return ParseStatus::Error;
...
void ChapterHtmlSlimParser::addLineToPage(std::shared_ptr<TextBlock> line, const uint32_t visibleOffset) {
  ...
  if (!currentPage) {
    if (!createPage("text_page")) return;
  }
  ...
  if (currentPageNextY + lineHeight > viewportHeight) {
    ...
    completePageFn(std::move(currentPage), xpathParagraphIndex, xpathListItemIndex, currentPageVisibleOffset);
    completedPageCount++;
    if (!createPage("text_page_break")) return;
  }
  setCurrentPageVisibleOffset(visibleOffset);
...
void ChapterHtmlSlimParser::makePages() {
  ...
  if (!currentPage) {
    if (!createPage("paragraph_page")) return;
  }

  if (!ensureMemory("paragraph_layout")) return;
