...
inline int esp_http_client_get_errno(esp_http_client_handle_t) { return 0; }
...
inline esp_err_t esp_http_client_get_url(esp_http_client_handle_t handle,
                                         char *url, int len) {
  if (!handle || !handle->config.url || !url || len <= 0)
    return ESP_FAIL;
  const size_t required = std::strlen(handle->config.url) + 1;
  if (required > static_cast<size_t>(len))
    return ESP_FAIL;
  std::memcpy(url, handle->config.url, required);
  return ESP_OK;
}
