...
class HalStorage {
 public:
  ...
  // Host-test fault injection. Firmware builds use lib/hal/HalStorage instead.
  void failRemovePathOnceForTest(std::string path, size_t matchingCallsToSkip = 0) {
    failRemovePath_ = std::move(path);
    failRemovePathSkips_ = matchingCallsToSkip;
  }
