...
void EpubReaderActivity::onEnter() {
  ...
  initialCoverSkipPending.store(
      ReaderUtils::shouldSkipInitialEpubCover(SETTINGS.skipEpubCoverPage != 0, static_cast<bool>(progress),
                                              initialClippingJump.has_value() || initialBookmarkJump.has_value()),
      std::memory_order_release);
  if (progress) {
    const size_t dataSize = progress.size;
    if (dataSize == ProgressFile::EPUB_LEGACY_PROGRESS_SIZE || dataSize == ProgressFile::EPUB_PROGRESS_SIZE ||
        dataSize == ProgressFile::EPUB_CONTENT_ANCHORED_PROGRESS_SIZE) {
      currentSpineIndex = data[0] + (data[1] << 8);
      nextPageNumber = data[2] + (data[3] << 8);
      cachedSpineIndex = currentSpineIndex;
      LOG_DBG("ERS", "Loaded cache: %d, %d", currentSpineIndex, nextPageNumber);
    }
    if (dataSize >= ProgressFile::EPUB_PROGRESS_SIZE) {
      cachedChapterTotalPageCount = data[4] + (data[5] << 8);
    }
    if (dataSize == ProgressFile::EPUB_CONTENT_ANCHORED_PROGRESS_SIZE) {
  ...
  // The OPF text reference is only a first-open hint. Spine zero is also a
  // valid saved reading position, so never replace it when progress exists.
  if (!progress && currentSpineIndex == 0) {
...
void EpubReaderActivity::onPause() {
  ...
  // the resident .cpfont and its glyph cache before those features allocate.
  sdFontSystem.releaseLoadedFont(renderer);
}

void EpubReaderActivity::onResume() {
  readerWaveform.beginTransition();
  // The per-book settings are still active. Restore their SD font only when
  // the actual reading page becomes active again.
...
void EpubReaderActivity::finishDeferredOpenState() {
  ...
  if (!skipStartupRecentUpdate) {
    RECENT_BOOKS.addBook(epub->getPath(), epub->getTitle(), epub->getAuthor(),
                         epub->getCoverItemHref().empty() ? std::string{} : epub->getThumbBmpPath());
  }
...
bool EpubReaderActivity::pumpImagePreparation() {
  if (!epub || SETTINGS.imageRendering != CrossPointSettings::IMAGES_DISPLAY ||
      (!deferredCoverFirstPageVisible && !imagePreparationForVisiblePage.load(std::memory_order_acquire))) {
    cancelImagePreparation();
    return false;
  }
...
void EpubReaderActivity::openReaderMenu() {
  ...
  startActivityForResult(
      std::make_unique<EpubReaderMenuActivity>(
          ...
          currentPageBookmarked, EpubReaderMenuActivity::ReaderKind::Epub, clippingStore.isLoaded(),
...
void EpubReaderActivity::loop() {
  ...
  {
    RenderLock lock(std::try_to_lock);
    if (!inputEdge && !readerInputHeld && pendingPageTurnDelta == 0 && lock.ownsLock() &&
        !activityManager.hasPendingRender() && section && !section->isBuilding() && section->isPartial() &&
  ...
  // watermark re-parses the whole chapter synchronously. Keep ticking until it finalizes.
  {
    ...
    if (!inputEdge && !readerInputHeld && lock.ownsLock() && !activityManager.hasPendingRender()) {
      const bool waitingForRequestedPage = sectionLandingPending || sectionRenderWaiting;
      const bool targetBuildRequired = waitingForRequestedPage && !requestedSectionTargetReady();
      bool landingWarmupActive = sectionLandingPending && sectionLandingWarmupPending &&
                                 requestedSectionTargetReady() && !sectionLandingReadyForRender();
      if (landingWarmupActive && pendingPageTurnDelta != 0) {
        // The requested page is already readable and a real turn is waiting.
        // Drop only the speculative landing runway, render the target, then
        // apply the queued turn before doing any more background layout.
        sectionLandingWarmupPending = false;
        landingWarmupActive = false;
        if (sectionRenderWaiting && requestedSectionPageReady()) requestUpdate();
      }
      const bool foregroundBuild = targetBuildRequired || landingWarmupActive;
      const bool withinBuildWindow =
          section && (foregroundBuild || section->isPartial() ||
                      static_cast<int>(section->pageCount) < section->currentPage + BUILD_WINDOW_AHEAD);
      bool buildAllowed = false;
      if (targetBuildRequired) {
        buildHeapPaused = false;
        buildAllowed = true;
      } else if (pendingPageTurnDelta != 0) {
        buildAllowed = false;
      } else if (landingWarmupActive) {
        buildAllowed = buildTickHeapGate();
        if (!buildAllowed) {
          // The requested page is already readable. Optional look-ahead must
          // not turn low heap into a failed landing or a wait that can never
          // finish; render the target and let later turns rebuild as needed.
          sectionLandingWarmupPending = false;
          if (sectionRenderWaiting && requestedSectionPageReady()) requestUpdate();
        }
      } else {
      ...
      if (section && section->isBuilding() && withinBuildWindow && buildAllowed) {
#if defined(ENABLE_SERIAL_LOG) && defined(LOG_LEVEL) && LOG_LEVEL >= 2
  ...
  // Coalesce a burst into its latest target. A chapter transition consumes
  // one logical step, then leaves the bounded remainder for the new section.
  if (ReaderUtils::queuedPageTurns(pendingPageTurnDelta) != 0 && !inputEdge && !readerInputHeld &&
      !activityManager.hasPendingRender()) {
    bool moved = false;
    bool boundaryTurn = false;
    bool forward = false;
    {
      RenderLock lock(std::try_to_lock);
      if (lock.ownsLock() && !activityManager.hasPendingRender()) {
        if (currentSpineIndex < 0 || currentSpineIndex >= epub->getSpineItemsCount()) {
          ReaderUtils::clearQueuedPageTurns(pendingPageTurnDelta);
        } else if (section && ReaderUtils::queuedPageTurns(pendingPageTurnDelta) < 0 && section->currentPage <= 0 &&
                   epub->getAdjacentLinearSpineIndex(currentSpineIndex, false) < 0) {
          // Repeated Back taps buffered at the first page have nowhere to go;
          // do not turn them into a series of identical e-ink redraws.
          ReaderUtils::clearQueuedPageTurns(pendingPageTurnDelta);
        } else if (section && !sectionLandingPending && !sectionRenderWaiting) {
          moved = retargetQueuedPageTurns();
          if (!moved && section->isBuildComplete() && ReaderUtils::queuedPageTurns(pendingPageTurnDelta) != 0) {
            const int boundaryDelta = ReaderUtils::takeQueuedPageTurns(pendingPageTurnDelta);
            forward = boundaryDelta > 0;
            boundaryTurn = boundaryDelta != 0;
          }
        }
      }
    }
  ...
  if (mappedInput.wasPressed(MappedInputManager::Button::Confirm)) {
    confirmHold.onPress();
    if (!automaticPageTurnActive && section && !(atEndOfBook && endOfBookOptions.menuActive())) {
  ...
  if (automaticPageTurnActive) {
    ...
    // Re-arm only the automatic timer while the destination is still landing
    // or a render owns the lock. Manual page gestures must continue below so
    // pageTurn() can queue them instead of dropping the input edge.
    ...
    if (!inputEdge && !readerInputHeld && !activityManager.hasPendingRender()) {
      {
        ...
        if (progressLock.ownsLock() && !activityManager.hasPendingRender() && pendingPageTurnDelta == 0 &&
            ...
            static_cast<uint32_t>(millis()) - pendingProgressSave.stagedAtMs >= PROGRESS_SAVE_IDLE_MS) {
          if (!flushPendingProgressSave()) {
            pendingSyncSaveError = true;
            requestUpdate();
          }
      ...
      // work gets the idle slice.
      if (imagePreparationForVisiblePage.load(std::memory_order_acquire)) {
        pumpImagePreparation();
      } else if (!section || sectionTurnBufferReady(section->currentPage)) {
        finishDeferredOpenState();
  ...
  if (longPress && SETTINGS.longPressButtonBehavior == SETTINGS.CHAPTER_SKIP) {
    initialCoverSkipPending.store(false, std::memory_order_release);
    {
      RenderLock lock(*this);
      if (sectionLandingPending || sectionRenderWaiting) return;
      clearDeferredReposition();
      pendingPageTurnDelta = 0;
      consumeReadingViewSignal();
      stopReadingPage(false, static_cast<uint32_t>(millis()));
...
void EpubReaderActivity::openClippingSelection() {
  ...
  // The first operation in ClipSelectionActivity reserves its bounded Word
  // table. Match CrossInk's proven 16 KiB contiguous safety margin so a
  // fragmented heap fails closed instead of reaching throwing vector growth.
  constexpr uint32_t CLIP_SELECTION_RESERVE_MARGIN = 16U * 1024U;
  static_assert(sizeof(ClipTextBuilder::Word) <= UINT32_MAX / ClipSelectionActivity::MAX_VISIBLE_WORDS);
  const uint32_t wordReserveBytes =
      static_cast<uint32_t>(sizeof(ClipTextBuilder::Word) * ClipSelectionActivity::MAX_VISIBLE_WORDS);
  const auto clipMemory = MemoryBudget::snapshot();
  if (!MemoryBudget::hasContiguousHeadroom(clipMemory.maxAllocHeap, wordReserveBytes, CLIP_SELECTION_RESERVE_MARGIN)) {
    LOG_ERR("CLIP", "Low heap for clipping selection (%u free, %u max alloc, need block %u); skipping",
            clipMemory.freeHeap, clipMemory.maxAllocHeap, wordReserveBytes + CLIP_SELECTION_RESERVE_MARGIN);
    pendingClippingNotice = ClippingNotice::Unavailable;
    requestUpdate();
    return;
  }
  ...
  auto clipSelection = makeUniqueNoThrow<ClipSelectionActivity>(
      renderer, mappedInput, std::move(page), SETTINGS.getReaderFontId(), marginLeft, marginTop, currentPage, pageCount,
      paragraphIndex, currentClippingLayoutFingerprint(), pageLoader, &clippingStore.entries(),
      static_cast<uint16_t>(currentSpineIndex));
  if (!clipSelection) {
    LOG_ERR("CLIP", "OOM: failed to allocate clipping selection activity");
    pendingClippingNotice = ClippingNotice::Unavailable;
    requestUpdate();
    return;
  }

  startActivityForResult(std::move(clipSelection), [this](const ActivityResult& result) {
...
void EpubReaderActivity::applyAutoPageTurnRuntime(const uint8_t seconds, const bool active) {
  ...
  if (statusBarHeight == 0 || statusBarHeight == UITheme::getInstance().getProgressBarHeight()) {
    // Preserve current reading position so we can restore after reflow.
    RenderLock lock(*this);
    // During an initial/deep landing, nextPageNumber/pendingPageJump already
    // holds the real target while section->currentPage is only a build
    // placeholder. Do not replace that target. A normal requested-page wait,
    // however, stores its target directly in currentPage and must preserve it.
    if (section && !sectionLandingPending) {
      rememberCurrentContentOffset();
      cachedSpineIndex = currentSpineIndex;
      cachedChapterTotalPageCount = section->pageCount;
      nextPageNumber = section->currentPage;
...
void EpubReaderActivity::updateAutoPageTurnPreference(const uint8_t seconds, const bool active) {
  const uint8_t normalizedSeconds = normalizeAutoPageTurnSeconds(seconds);
  const bool settingsChanged =
      setPerBookAutoPageTurnState(bookReaderSettings, normalizedSeconds, active && normalizedSeconds != 0);
  applyAutoPageTurnRuntime(normalizedSeconds, active);
...
void EpubReaderActivity::pageTurn(const bool isForwardTurn, const bool queueWhileWaiting,
  ...
  if (queueWhileWaiting && !drainingQueuedTurn) {
    ...
    ReaderUtils::logPageTurnMetric("epub", "input", turnSequence, isForwardTurn ? 1 : -1, -1, -1,
...
#endif
  {
    ...
    if (!lock.ownsLock()) {
      if (queueWhileWaiting) {
        ReaderUtils::queuePageTurns(pendingPageTurnDelta, isForwardTurn ? 1 : -1);
        requestUpdate();
      }
#if defined(ENABLE_SERIAL_LOG) && defined(LOG_LEVEL) && LOG_LEVEL >= 2
      if (queueWhileWaiting) {
        ReaderUtils::logPageTurnMetric("epub", "queued", turnSequence, isForwardTurn ? 1 : -1, -1, -1,
                                       ReaderUtils::queuedPageTurns(pendingPageTurnDelta),
    ...
    if (sectionLandingPending || sectionRenderWaiting || sectionTransition ||
        (!drainingQueuedTurn && ReaderUtils::queuedPageTurns(pendingPageTurnDelta) != 0)) {
      if (queueWhileWaiting) {
        ReaderUtils::queuePageTurns(pendingPageTurnDelta, isForwardTurn ? 1 : -1);
        requestUpdate();
      }
...
bool EpubReaderActivity::retargetQueuedPageTurns() {
  if (!section) return false;
  const int delta = ReaderUtils::takeQueuedPageTurns(pendingPageTurnDelta);
  if (delta == 0) return false;

  const bool forward = delta > 0;
  int remaining = forward ? delta : -delta;
  bool moved = false;
  while (remaining > 0) {
    // Keep at most one requested page beyond an incomplete pagination
    // watermark. The rest remains queued until the builder proves whether the
    // section continues or the next logical turn crosses a chapter boundary.
    if (forward && (section->isBuilding() || section->isPartial()) &&
        section->currentPage >= static_cast<int>(section->pageCount)) {
      break;
    }
    if (!moveOnePageWithoutRendering(forward)) break;
    moved = true;
    --remaining;
    if (!section) break;
  }
  if (remaining > 0) ReaderUtils::queuePageTurns(pendingPageTurnDelta, forward ? remaining : -remaining);
  if (!moved) return false;

  clearDeferredReposition();
  consumeReadingViewSignal();
  stopReadingPage(forward, static_cast<uint32_t>(millis()));
  coverSkipHops = 0;
  const bool preparationMatchesCurrentPage = section && imagePrefetchSpine == currentSpineIndex &&
                                             imagePrefetchPage == section->currentPage &&
                                             imagePrefetchSectionGeneration == sectionGeneration;
  imagePreparationForVisiblePage.store(preparationMatchesCurrentPage && !imagePrefetchPageComplete,
                                       std::memory_order_release);
  refreshEstimatedTimeLeft();
  lastPageTurnTime = millis();
  return true;
...
bool EpubReaderActivity::skipCoverPageIfNeeded(const Page& page) {
  ...
  if (epub && page.hasImages()) {
    LOG_DBG("ERS", "Cover check: enabled=%u cover_only=%u spine=%d page=%d href=%s", skipEnabled ? 1U : 0U,
            coverOnly ? 1U : 0U, currentSpineIndex, section ? section->currentPage : nextPageNumber,
            epub->getCoverItemHref().c_str());
  }
#endif
  if (!initialCoverSkipPending.load(std::memory_order_acquire) || !skipEnabled || !epub || !coverOnly) {
    coverSkipHops = 0;
    return false;
  }
  ...
  const bool moved = moveOnePageWithoutRendering(true);
  if (!moved) {
    initialCoverSkipPending.store(false, std::memory_order_release);
    coverSkipHops = 0;
    return false;
...
bool EpubReaderActivity::sectionTurnBufferReady(const int targetPage) const {
  if (!section) return false;
  if (section->isBuildComplete()) return true;
  return targetPage >= 0 && targetPage + BUILD_WINDOW_AHEAD <= static_cast<int>(section->pageCount);
}

std::optional<int> EpubReaderActivity::sectionLandingTargetPage() const {
  if (!section || pendingPercentJump) return std::nullopt;

  int target = pendingPageJump.has_value() ? *pendingPageJump : std::max(0, nextPageNumber);
  if (pendingBookmarkSourceOffset.has_value() ||
      (cachedContentSourceOffset.has_value() && currentSpineIndex == cachedSpineIndex)) {
    const auto sourcePage = section->sourceOffsetTargetPage();
    if (!sourcePage.has_value()) return std::nullopt;
    target = *sourcePage;
  } else if (cachedVisibleTextOffset.has_value() && currentSpineIndex == cachedSpineIndex) {
    const auto visiblePage = section->getPageForVisibleTextOffset(*cachedVisibleTextOffset);
    if (!visiblePage.has_value()) return std::nullopt;
    target = *visiblePage;
  }
  if (!pendingAnchor.empty()) {
    const auto anchorPage = section->findAnchor(pendingAnchor);
    if (!anchorPage.has_value()) return std::nullopt;
    target = *anchorPage;
  }
  return target;
}

bool EpubReaderActivity::sectionLandingReadyForRender() const {
  if (!sectionLandingReady()) return false;
  if (!sectionLandingWarmupPending || !section || section->isBuildComplete()) return true;
  const auto target = sectionLandingTargetPage();
  return target.has_value() && sectionTurnBufferReady(*target);
}

bool EpubReaderActivity::requestedSectionTargetReady() const {
  if (!section) return false;
...
bool EpubReaderActivity::requestedSectionPageReady() const {
  if (sectionLandingPending) return sectionLandingReadyForRender();
  return requestedSectionTargetReady();
}
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  if (renderBlockingFeedbackOverlay()) {
    cancelImagePreparation();
    return;
  }
  if (!epub) {
  ...
  if (sectionLandingPending) {
    if (!sectionLandingReadyForRender()) {
  ...
  if (!section) {
    ...
    const bool visibleReposition = !cacheComplete && !sourceReposition && cachedVisibleTextOffset.has_value() &&
                                   currentSpineIndex == cachedSpineIndex;
    const bool contentReposition = sourceReposition || visibleReposition;
    if (sourceReposition) {
    ...
    if (!cacheComplete) {
      ...
      const auto partialAnchorPage = anchorJump ? section->getPageForAnchor(pendingAnchor) : std::nullopt;
      const bool partialTargetAvailable =
          section->isPartial() && !pendingPercentJump && !contentReposition &&
          (anchorJump ? partialAnchorPage.has_value() : target < static_cast<int>(section->pageCount));
      const int partialTarget = partialAnchorPage.has_value() ? *partialAnchorPage : target;
      // A target well inside a partial is already serviceable. Near the saved
      // watermark, start the rebuild now and spend one small opening slice on
      // it; otherwise the very next turn would have to restart from page zero.
      // The landing itself never waits for that rebuild to catch the watermark.
      const bool partialCoversLanding =
          partialTargetAvailable && partialTarget + PARTIAL_REBUILD_START_MARGIN < static_cast<int>(section->pageCount);
      if (partialCoversLanding) {
        ...
        const uint16_t debugPagesBefore = section->debugBuiltPageCount();
#endif
        // Spend one bounded opening slice on pagination runway. Fresh/missing
        // targets keep pumping below until their target-relative buffer exists;
        // a cached partial target can render after this slice while rebuilding
        // continues with page turns taking priority.
  ...
  if (retargetQueuedPageTurns() && !section) {
    requestUpdate();
    return;
  }
...
#endif
    const std::optional<bool> renderResult =
        renderContents(std::move(p), orientedMarginTop, orientedMarginRight, orientedMarginBottom, orientedMarginLeft,
                       &renderedPageFingerprint);
    if (!renderResult.has_value()) {
      return;
    }
    clippingHighlightsTruncated = *renderResult;
    if (readerOpenStagesPending) {
...
bool EpubReaderActivity::applyDeferredReposition() {
  if (!section || (!cachedContentSourceOffset.has_value() && !cachedVisibleTextOffset.has_value() &&
                   cachedChapterTotalPageCount == 0)) {
    return false;
  }
  ...
  const std::optional<uint16_t> visiblePage = !contentPage.has_value() && cachedVisibleTextOffset.has_value()
                                                  ? section->getPageForVisibleTextOffset(*cachedVisibleTextOffset)
                                                  : std::nullopt;
  if (section->isBuilding() && !contentPage.has_value() && !visiblePage.has_value()) return false;
...
void EpubReaderActivity::clearDeferredReposition() {
  ...
  cachedVisibleTextOffset.reset();
  cachedChapterTotalPageCount = 0;
  if (section) section->clearSourceOffsetTarget();
}
...
void EpubReaderActivity::stageProgressSave(const int spineIndex, const int currentPage, const int pageCount) {
  ...
  pendingProgressSave = {true, false, spineIndex, currentPage, pageCount, offset, static_cast<uint32_t>(millis())};
}
...
std::optional<bool> EpubReaderActivity::renderContents(std::unique_ptr<Page> page, const int orientedMarginTop,
                                                       const int orientedMarginRight, const int orientedMarginBottom,
                                                       const int orientedMarginLeft,
                                                       uint32_t* const pageFingerprintOut) {
  ...
  // Image pages bypass the ordinary refresh helper. On X3, defer the same fast
  ...
  renderStatusBar();
  if (retargetQueuedPageTurns()) {
    if (manualRefreshPending) forcedRefreshPending = true;
    return std::nullopt;
  }
  // The BW pass marks a missing/pending raw image using only Page-local RAM.
  // Avoid an O(N) SD/cache probe before rendering: valid pixel caches display
  // immediately, while raw materialization resumes cooperatively after this
  // fast placeholder frame.
  if (page->hasImagesAwaitingRawPreparation()) {
    queueVisiblePageImagePreparation();
  ...
  if (pageHasImages && supportsGrayscale && !imageCacheFallback) {
    ...
    if (x3ImageCleanupPending) {
      ...
      // A non-cleaned grayscale image can leave gray charge in the image
      // region. Clean the next ordinary page before resuming the cadence.
      pagesUntilFullRefresh = 1;
    }
  } else {
    ReaderUtils::displayWithRefreshCycle(renderer, pagesUntilFullRefresh, readerWaveform);
