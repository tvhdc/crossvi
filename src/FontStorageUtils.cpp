...
bool removeDirectoryIfPresent(const char* path) {
...
bool isValidFamilyName(const char* name) {
  return FontStorageContract::isValidFamilyName(name);
}

bool isValidCpfontFilename(const char* name) {
  return FontStorageContract::isValidCpfontFilename(name);
}
...
bool buildFontPath(const char* root, const char* family, const char* filename, char* output,
